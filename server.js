...
app.post('/api/bookings/request',async(req,res)=>{
  try{
    ...
    if(apt){await db().from('loyalty_visits').insert({client_id:client.id,appointment_id:apt.id,points:1}).catch(()=>{});await db().from('clients').update({total_visits:(client.total_visits||0)+1,total_spent_cents:(client.total_spent_cents||0)+priceCents,loyalty_points:(client.loyalty_points||0)+1}).eq('id',client.id);}
