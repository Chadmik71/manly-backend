...
async function sendEmail({to,subject,html}){ if(!process.env.RESEND_API_KEY)return{ok:false}; try{const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':'Bearer '+process.env.RESEND_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({from:'Manly Massage <noreply@manlyremedialthai.com.au>',to,subject,html})});return await r.json();}catch(e){console.error('Email:',e);return{ok:false};} }
